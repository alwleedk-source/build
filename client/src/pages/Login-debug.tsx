...
export default function Login() {
  ...
  const loginMutation = trpc.admin.login.useMutation({
    onSuccess: (data) => {
      ...
      addDebug(`User: ${JSON.stringify(data)}`);
      setLocation("/admin");
    },
    onError: (error) => {
      addDebug(`❌ Login error: ${error.message}`);
      addDebug(`Error code: ${error.data?.code || 'unknown'}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email || !password) {
      addDebug('⚠️  Validation failed: empty fields');
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 p-4">
      <Card className="w-full max-w-md shadow-xl">
        <CardHeader className="space-y-4 text-center">
          ...
          <div>
            <CardTitle className="text-2xl font-bold">Inloggen (Debug Mode)</CardTitle>
            <CardDescription>
              {APP_TITLE} Dashboard
            </CardDescription>
          </div>
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    addDebug(`Email changed: ${e.target.value}`);
                  }}
                  disabled={isLoading}
                  className="pl-10"
                  required
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    addDebug(`Password changed (length: ${e.target.value.length})`);
                  }}
                  disabled={isLoading}
                  className="pl-10"
                  required
